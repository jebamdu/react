...
const Exams = () => {
    ...
    return (<><h1 className='heading'>
        ...
        {levelID && <>{" > "}<Link to={`/admin/exams/${levelID}`}>level</Link></>}
        {catID && <>{" > "}<Link to={`/admin/exams/${levelID}/${catID}`}>Type</Link></>}
        {catID && <button className='addques btn' onClick={addQus}>Add question</button>}
    </h1>
        <div className="exams">
            <Table data={exams} />
